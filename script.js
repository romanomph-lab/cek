    ...
    window.importOngkirFromSheets = async function() {
        const sheetsUrl = prompt('Masukkan URL Google Sheets (format: https://docs.google.com/spreadsheets/d/SHEET_ID/edit)');
        if (!sheetsUrl) return;

        try {
            const sheetId = sheetsUrl.match(/\/d\/([a-zA-Z0-9-_]+)\//)?.[1];
            if (!sheetId) return alert('URL tidak valid.');

            const csvUrl = `https://docs.google.com/spreadsheets/d/${sheetId}/export?format=csv&gid=0`;
            
            const response = await fetch(csvUrl);
            if (!response.ok) throw new Error('Gagal mengakses sheet');

            const csv = await response.text();
            const lines = csv.split('\n').filter(l => l.trim());
            
            if (lines.length < 2) return alert('Sheet kosong atau format tidak valid.');

            const headers = lines[0].split(',').map(h => h.trim().toLowerCase());
            const wilayahIdx = headers.indexOf('wilayah');
            const tarifIdx = headers.indexOf('tarif');

            if (wilayahIdx === -1 || tarifIdx === -1) {
                return alert('Kolom harus bernama "wilayah" dan "tarif".');
            }

            // Ambil data existing di database
            const existingSnap = await get(ref(db, 'ongkir_wilayah'));
            const existingData = existingSnap.val() || {};
            const existingMap = {};
            
            Object.entries(existingData).forEach(([key, item]) => {
                const normalized = normalizeNama(item.wilayah);
                existingMap[normalized] = { key, data: item };
            });
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                const wilayah = cols[wilayahIdx]?.trim();
                const tarifStr = cols[tarifIdx]?.trim();
                
                if (!wilayah || !tarifStr) continue;

                const tarif = parseInt(tarifStr.replace(/[^0-9]/g, '')) || 0;
                if (tarif <= 0) continue;

                const normalized = normalizeNama(wilayah);
                const payload = { wilayah, tarif, syncedAt: new Date().toISOString() };

                if (existingMap[normalized]) {
                    // Update data yang sudah ada
                    const existingKey = existingMap[normalized].key;
                    await update(ref(db, `ongkir_wilayah/${existingKey}`), payload);
                    updated++;
                } else {
                    // Tambah data baru
                    await set(push(ref(db, 'ongkir_wilayah')), payload);
                    imported++;
                }
            }

            alert(`${imported} data baru diimport, ${updated} data diupdate dari Sheets.`);
        } catch (err) {
            alert('Error import: ' + err.message);
        }
    };

    window.setupAutoSyncSheets = async function() {
        const sheetsUrl = prompt('Masukkan URL Google Sheets untuk auto-sync:\n(Akan otomatis update setiap 5 menit)');
        if (!sheetsUrl) return;

        const sheetId = sheetsUrl.match(/\/d\/([a-zA-Z0-9-_]+)\//)?.[1];
        if (!sheetId) return alert('URL tidak valid.');

        localStorage.setItem('sheets_sync_id', sheetId);
        localStorage.setItem('sheets_sync_enabled', 'true');

        alert('Auto-sync Sheets diaktifkan! Akan update setiap 5 menit.');
        startAutoSyncSheets();
    };

    window.startAutoSyncSheets = async function() {
        const sheetId = localStorage.getItem('sheets_sync_id');
        const isEnabled = localStorage.getItem('sheets_sync_enabled') === 'true';

        if (!sheetId || !isEnabled) return;
        ...
        const syncInterval = setInterval(async () => {
            try {
                ...
                if (wilayahIdx === -1 || tarifIdx === -1) return;
                ...
                for (let i = 1; i < lines.length; i++) {
                    ...
                    if (existingMap[normalized]) {
                        const existingKey = existingMap[normalized].key;
                        const existingTarif = existingMap[normalized].data.tarif;
                        
                        // Update hanya jika ada perubahan
                        if (existingTarif !== tarif) {
                            await update(ref(db, `ongkir_wilayah/${existingKey}`), payload);
                            changeCount++;
                        }
                    } else {
                ...
                if (changeCount > 0) {
                    console.log(`[Auto-Sync Sheets] ${changeCount} perubahan terdeteksi dan diupdate`);
                }
            } catch (err) {
        ...
        localStorage.setItem('sheets_sync_interval', syncInterval);
    };

    window.stopAutoSyncSheets = function() {
        localStorage.setItem('sheets_sync_enabled', 'false');
        alert('Auto-sync Sheets dihentikan.');
    };

    // Jalankan auto-sync saat aplikasi load jika sudah diaktifkan sebelumnya
    if (localStorage.getItem('sheets_sync_enabled') === 'true') {
        setTimeout(() => window.startAutoSyncSheets(), 2000);
    }
// Tambahkan fungsi toast setelah deklarasi variabel global
...
window.exportOngkirToExcel = function() {
    const data = [];
    let nomorUrut = 1;

    Object.entries(cloudOngkirList || {}).forEach(([key, ongkir]) => {
        if (!ongkir || !ongkir.wilayah) return;

        data.push({
            No: nomorUrut,
            'Wilayah': ongkir.wilayah || '-',
            'Tarif': ongkir.tarif || 0,
            'Tarif Format': `Rp ${(ongkir.tarif || 0).toLocaleString('id-ID')}`
        });
    ...
    if (!data.length) {
        alert('Tidak ada data ongkir untuk diexport.');
        return;
    }
    ...
    for (let C = range.s.c; C <= range.e.c; ++C) {
        ...
        ws[cellRef].s = {
            ...
            fill: { patternType: 'solid', fgColor: { rgb: '0066FF' } },
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Daftar Ongkir');
    XLSX.writeFile(wb, `Daftar_Ongkir_${new Date().toISOString().split('T')[0]}.xlsx`);
    
    window.toast('Export daftar ongkir berhasil diunduh');
};
